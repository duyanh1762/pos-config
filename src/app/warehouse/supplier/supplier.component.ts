...
export class SupplierComponent implements OnInit {
  ...
  async load(){
    await this.api.supplier({mode:"get",data:""}).toPromise().then((res:any)=>{
      this.supps = res;
      this.suppsLU = res;
    });
  }
  addSupplier(){
    this.bsMS.show(AddSupplierComponent,{
      initialState:{
        data:{
          type:"create",
          sup:""
  ...
  search(){
        ...
        if(value.length <= 0){
          ...
          this.suppsLU.forEach((s:Supplier)=>{
            if(this.api.removeAccents(s.name).toLowerCase().indexOf(this.api.removeAccents(value.toLowerCase())) != -1){
              this.supps.push(s);
            }
          });
        }
